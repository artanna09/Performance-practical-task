...
object App extends SparkSupport {
  ...
  def optimalCode1(text: RDD[String]): (Array[String], Array[String]) = {
    ...
    val groupedCodes = codes.reduceByKey(_+_)

    val sortedGroups = groupedCodes.sortBy(_._2, ascending = false)

    val top3Groups = sortedGroups.map(_._1).take(3)

    val sortedGroupsAsc = groupedCodes.sortBy(_._2)

    val bottom3Groups: Array[String] = sortedGroupsAsc.map(_._1).take(3)
  ...
  def optimalCode11(text: RDD[String]): (Array[String], Array[String]) = {
    ...
    def seqOp = (element1: Int, element: Int) => element1 + element

    def combOp = (element1: Int, element: Int) => element + element1


    val groupedCodes = codes.aggregateByKey(0)(seqOp, combOp)
  ...
  def optimalCode2(crimesDb: RDD[String], commitedCrimes: RDD[String]): Unit = {
    ...
    def addCommitedCrimes(commited: RDD[String]) = {
      ...
      val result: RDD[String] = commited
  ...
  def main(args: Array[String]): Unit = {
    ...
    val (top0, bottom0) = time {
      unoptimalCode1(crimeCategories)
    }
    println(s"""( ${ top0.mkString(",") } ... ${ bottom0.reverse.mkString(",") })""")
